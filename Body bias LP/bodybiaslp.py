...
def subtiles(tile_size,mapping):
    ...
    subtiles_group = []
    for i in range(0,mapping_col,tile_col):
        for j in range(0,mapping_row,tile_row):
...
def bbdlp(mapping, BBD_mapping, cons_table, BBD_data, BBD_voltages, all_paths, cleaned_tasks, LB, UB, MAX_LAT, tile_size, cgra_size):
    ...
    zero_power = 0
    for elem in cons_table:
        zero_power += cons_table[elem][0.0]['ibb']

    zero_power = zero_power #- (len(cons_table)-len(tasks)) * BBD_data['NOP'][0.0]['ibb']

    for v in prob.variables():
        if v.varValue != 0:
            print(v.name, "=", v.varValue,)
    print("Zero bias latency:", MAX_LAT)
    print("Zero bias power:", zero_power)
    # print(zero_power)
    ...
    print(value(prob.objective)+(96-len(BBD_mapping))*BBD_data['NOP'][-1.0]['ibb'])
    print("Time:", toc)
